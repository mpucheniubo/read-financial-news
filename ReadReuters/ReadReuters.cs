...
namespace ReadReuters
{
    public static class ReadReuters
    {
        [FunctionName("ReadReuters")]
        ...
        public static List<Newsarticle> ReadNews(ILogger log)
        {
            ...
            foreach (string articleUrl in articleUrls)
            {
                ...
                article.lang = "de";
                article.dataSite = "reuters";
                article.dataRegion = "DE";
                article.url = "https://de.reuters.com" + articleUrl;
                ...
                // get image link
                var pics = doc.DocumentNode.SelectNodes("//link[@rel]");

                foreach (HtmlNode pic in pics)
                {
                    if (pic.Attributes["rel"].Value == "image_src")
                    {
                        article.articleImageUrl = pic.Attributes["href"].Value;
                    }
                }
                ...
                foreach (var node in list)
                {
                    ...
                    if (node.GetAttributeValue("name", "") == "analyticsAttributes.contentTitle")
                    {
                    ...
                    if (node.GetAttributeValue("name", "") == "analyticsAttributes.articleDate")
                    {
                        article.articlePublished = node.GetAttributeValue("content", "");
                    }
                    if (node.GetAttributeValue("name", "") == "REVISION_DATE")
                    {
                ...
                var div = doc.DocumentNode.SelectSingleNode("//div[@class='StandardArticleBody_body']");
                if (div != null)
                {
        ...
        public static string[] getArticleUrls()
        {
            ...
            var url = "https://de.reuters.com/news/archive/topNews";
            var web = new HtmlWeb();
            var doc = web.Load(url);



            var div = doc.DocumentNode.SelectSingleNode("//div[@class='news-headline-list  ']");
            if (div != null)
            {
        ...
        public static void WriteNews(ILogger log, List<Newsarticle> articleList)
        {
            ...
            DateTime articleUpdated;
            List<string> NewsUrl = new List<string>();

            string DateTimeFormat = "ddd MMM dd HH:mm:ss UTC yyyy";
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(SqlConnectionString))
                {
                    ...
                    sb.Append("SELECT DISTINCT [Url] FROM [News].[Values] WHERE [DataSite] = 'reuters' AND [RowCreated] > DATEADD(DAY,-" + DaysAgo.ToString() + ",SYSUTCDATETIME())");
                            ...
                            using (SqlCommand command = new SqlCommand(sqlInput, connection))
                            {
                                ...
                                if (!string.IsNullOrEmpty(article.articleUpdated))
                                {
                                    if (DateTimeOffset.TryParse(article.articlePublished, out articlePublished))
                                    {
                                        ...
                                        if (DateTime.TryParseExact(article.articleUpdated, DateTimeFormat, CultureInfo.InvariantCulture, DateTimeStyles.None, out articleUpdated))
                                        {
                                            command.Parameters.AddWithValue("@ArticleUpdated", new SqlDateTime(articleUpdated));
                                        }
                                        else
                                        {
                                    ...
                                    else
                                    {
                                        if (DateTime.TryParseExact(article.articleUpdated, DateTimeFormat, CultureInfo.InvariantCulture, DateTimeStyles.None, out articleUpdated))
                                        {
                                            command.Parameters.AddWithValue("@ArticlePublished", new SqlDateTime(articleUpdated));
                                            command.Parameters.AddWithValue("@ArticleUpdated", new SqlDateTime(articleUpdated));
                                        }
                                        else
                                        {

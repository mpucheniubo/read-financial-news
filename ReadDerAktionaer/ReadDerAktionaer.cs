...
namespace ReadDerAktionaer
{
    public static class ReadDerAktionaer
    {
        [FunctionName("ReadDerAktionaer")]
        ...
        public static List<Newsarticle> ReadNews(ILogger log)
        {
            ...
            foreach (string articleUrl in articleUrls)
            {
                ...
                article.lang = "de";
                article.dataSite = "Der Aktionaer";
                article.dataRegion = "DE";
                article.url = "https://www.deraktionaer.de" + articleUrl;
                ...
                var div = doc.DocumentNode.SelectSingleNode("//div[@id='article-body']");
                if (div != null)
                {
        ...
        public static string[] getArticleUrls()
        {
            ...
            var url = "https://www.deraktionaer.de";
            var web = new HtmlWeb();
            var doc = web.Load(url);



            var div = doc.DocumentNode.SelectSingleNode("//div[@id='index-article-list']");
            if (div != null)
            {
        ...
        public static void WriteNews(ILogger log, List<Newsarticle> articleList)
        {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(SqlConnectionString))
                {
                    ...
                    sb.Append("SELECT DISTINCT [Url] FROM [News].[Values] WHERE [DataSite] = 'Der Aktionaer' AND [RowCreated] > DATEADD(DAY,-" + DaysAgo.ToString() + ",SYSUTCDATETIME())");

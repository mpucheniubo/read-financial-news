...
namespace ReadBoerseOnline
{
    public static class ReadBoerseOnline
    {
        [FunctionName("ReadBoerseOnline")]
        ...
        public static List<Newsarticle> ReadNews(ILogger log)
        {
            ...
            foreach (string articleUrl in articleUrls)
            {
                ...
                article.lang = "de";
                article.dataSite = "boerse-online.de";
                article.dataRegion = "DE";
                article.url = "https://www.boerse-online.de" + articleUrl;
                ...
                foreach (var node in list)
                {
                    ...
                    if (node.GetAttributeValue("name", "") == "page-topic")
                    {
                        article.articleSection = node.GetAttributeValue("content", "");
                    }
                    if (node.GetAttributeValue("name", "") == "date")
                    {
                        article.articlePublished = DateTime.Now.ToString(); // gibt kein gutes datum auf der seite
                    }
                ...
                var div = doc.DocumentNode.SelectSingleNode("//div[@class='teaser']");
                if (div != null)
                {
                    article.articleSummary = div.InnerText.Replace("\r\n", "");
                }
                ...
                .ToList()
                .ForEach(n => n.Remove());
                div = doc.DocumentNode.SelectSingleNode("//div[@class='content news_detail']");
                if (div != null)
                {
                    article.text = div.OuterHtml;
                    article.text = article.text.Replace("<div class=\"content news_detail\">", "");
                    article.text = article.text.Replace("<div class=\"teaser\">", "");
                    article.text = article.text.Replace("<div id=\"Ads_BA_CAD2\" class=\"\" style=\"text - align:center; \">", "");
                    article.text = article.text.Replace("<div id=\"responsive_ad\">", "");
                    article.text = article.text.Replace("<div class=\"image_source\">", "");
                    article.text = article.text.Replace("<div class=\"newsEnd\">", "");
                    article.text = article.text.Replace("</div>", "");

                    while (true)
                    {
                        int index = article.text.IndexOf("<div");
                        if (index != -1)
                        {
                            string temp = article.text.Substring(index);
                            string part1 = article.text.Substring(0, index);
                            index = temp.IndexOf(">");
                            string part2 = temp.Substring(index + 1);
                            article.text = part1 + part2;
                        }
                        else
                        {
                            break;
        ...
        public static string[] getArticleUrls()
        {
            ...
            var url = "https://www.boerse-online.de/nachrichten/top-news";
            var web = new HtmlWeb();
            var doc = web.Load(url);



            var div = doc.DocumentNode.SelectNodes("//table[@class='table']").First();
            if (div != null)
            {
        ...
        public static void WriteNews(ILogger log, List<Newsarticle> articleList)
        {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(SqlConnectionString))
                {
                    ...
                    sb.Append("SELECT DISTINCT [Url] FROM [News].[Values] WHERE [DataSite] = 'boerse-online.de' AND [RowCreated] > DATEADD(DAY,-" + DaysAgo.ToString() + ",SYSUTCDATETIME())");

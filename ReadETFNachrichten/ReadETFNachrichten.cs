...
namespace ReadETFNachrichten
{
    public static class ReadETFNachrichten
    {
        [FunctionName("ReadETFNachrichten")]
        ...
        public static List<Newsarticle> ReadNews(ILogger log)
        {
            ...
            foreach (string articleUrl in articleUrls)
            {
                ...
                article.lang = "de";
                article.dataSite = "ETF Nachrichten";
                article.dataRegion = "DE";
                article.url = "https://www.etf-nachrichten.de" + articleUrl;
                article.articleSection = "Economics,business,Wirtschaft,ETF";
                ...
                {
                    if (node.GetAttributeValue("property", "") == "twitter:creator")
                    {
                        article.author = node.GetAttributeValue("content", "").Replace("@", "").Replace("_", " ");
                    }
                    if (node.GetAttributeValue("property", "") == "og:description")
                    {
                        article.articleSummary = node.GetAttributeValue("content", "");
                    }
                    if (node.GetAttributeValue("property", "") == "article:section")
                    {
                    ...
                    if (node.GetAttributeValue("property", "") == "article:published_time")
                    {
                        article.articlePublished = node.GetAttributeValue("content", "");
                    }
                    if (node.GetAttributeValue("property", "") == "og:updated_time")
                    {
                        article.articleUpdated = node.GetAttributeValue("content", "");
                    }
                    if (node.GetAttributeValue("property", "") == "og:image:secure_url")
                    {
                ...
                var div = doc.DocumentNode.SelectNodes("//div[@class='tdb-block-inner td-fix-index']")[3];
                if (div != null)
                {
                    article.text = div.InnerText.Replace("TippInteresse an ETF ? Jetzt zum Newsletter anmelden oder die besten Anbieter im Depotvergleich oder ETF - Sparplan - Vergleich finden!", "").Replace("Startseite", "");
                }
        ...
        public static string[] getArticleUrls()
        {
            ...
            var url = "https://www.etf-nachrichten.de/thema/news/";
            var web = new HtmlWeb();
            var doc = web.Load(url);


            var div = doc.DocumentNode.SelectSingleNode("//div[@class='td-main-content-wrap td-container-wrap']");
            if (div != null)
            {
        ...
        public static void WriteNews(ILogger log, List<Newsarticle> articleList)
        {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(SqlConnectionString))
                {
                    ...
                    sb.Append("SELECT DISTINCT [Url] FROM [News].[Values] WHERE [DataSite] = 'ETF Nachrichten' AND [RowCreated] > DATEADD(DAY, -" + DaysAgo.ToString() + ", SYSUTCDATETIME())");

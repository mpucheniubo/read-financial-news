...
namespace ReadBoerseGlobal
{
    public static class ReadBoerseGlobal
    {
        [FunctionName("ReadBoerseGlobal")]
        ...
        public static List<Newsarticle> ReadNews(ILogger log)
        {
            ...
            foreach (string articleUrl in articleUrls)
            {
                ...
                {
                    if (node.GetAttributeValue("property", "") == "og:site_name")
                    {
                    ...
                    if (node.GetAttributeValue("property", "") == "article:tag")
                    {
                        article.keywords += "," + node.GetAttributeValue("content", "");
                    }
                }
                ...
                var div = doc.DocumentNode.SelectSingleNode("//div[@class='entry-content clearfix']");
                if (div != null)
                {
                    article.text = div.InnerText.Replace("Teilen", "").Replace("mit:", "").Replace("FacebookTwitter", "").Replace("DruckenGefällt", "").Replace("mir:", "").Replace("Gefällt mir", "").Replace("Wird geladen...", "").Replace("Ähnliche Beiträge", "");
                }
        ...
        public static string[] getArticleUrls()
        {
            ...
            var url = "https://www.boerse-global.de/category/news";
            var web = new HtmlWeb();
            var doc = web.Load(url);



            var div = doc.DocumentNode.SelectSingleNode("//ul[@class='vw-breaking-news-list list-unstyled']");
            if (div != null)
            {
        ...
        public static void WriteNews(ILogger log, List<Newsarticle> articleList)
        {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(SqlConnectionString))
                {
                    ...
                    sb.Append("SELECT DISTINCT [Url] FROM [News].[Values] WHERE [DataSite] = 'Börse Global' AND [RowCreated] > DATEADD(DAY,-" + DaysAgo.ToString() + ",SYSUTCDATETIME())");
